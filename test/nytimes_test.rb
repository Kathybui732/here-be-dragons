...
class NytimesTest < Minitest::Test
  ...
  def test_it_can_get_all_stories_with_subsection_of_politics
    #Set your code to the local variable, "result"
    # result = []
    # @hash[:results].each do |hash_result|
    #   result << hash_result if hash_result[:subsection] == "Politics"
    # end

    result = @hash[:results].select do |hash_result|
      hash_result[:subsection] == "Politics"
    end
